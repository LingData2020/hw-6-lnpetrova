...
summary(fit2)
```
Yes, _consonansts_ variable is still significant. Its p-value increased after _area_ random effect was added (0.000905 > 1.63e-06). The lines are parallel, they differ from each other by a shift. It is defined by the corresponding area value to which the random effect was added.
...
summary(fit3)
```
The variable _consonants_ is not significant anymore because p-value became higher than a significance level. Moreover, the lines are not parallel anymore because the coefficient of the variable was changed by adding a random slope.
```
...
summary(fit4)
```
The variable _consonants_ is still not significant because of its p-value, which is higher than a significance level. As for the third model, the prediction lines are different comparing to the first and second models, as each group of the third model has its intercept and coefficient of the variable. In this case, lines are not parallel, and each one has its slope. 
